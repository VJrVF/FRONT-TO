...
function comprarButtonClicked() {
  const shoppingCartItems = getItemInShoppingCart();
...
function getItemInShoppingCart(){
  const shoppingCartItems = document.querySelectorAll('.shoppingCartItem');
  const arrShoppingCartItems = [];

  shoppingCartItems.forEach( sci => {
    const sciQuantity = sci.querySelector('.shoppingCartItemQuantity');
    const sciQuantityNum = Number(sciQuantity.value);
    const itemId = sci.dataset.id;
